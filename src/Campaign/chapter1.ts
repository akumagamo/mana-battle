...
export default async (scene: Phaser.Scene, unit: Unit) => {
  ...
  const playerStartingPosition = map.cities.find((c) => c.id === "castle1");
  if (!playerStartingPosition) throw new Error(INVALID_STATE);

  const squadIndex = map.squads.merge(
    Map(squads.map((s) => [s.id, toMapSquad(s, playerStartingPosition)]))
  );
  const unitIndex = map.units.merge(alliedUnits);
  const state = {
    ...map,
    dispatchedSquads: Set(List(["1"]).concat(map.squads.keySeq())),
    squads: squadIndex,
    units: unitIndex,
    unitSquadIndex: createUnitSquadIndex(squadIndex.map((s) => s.squad)),
  };
