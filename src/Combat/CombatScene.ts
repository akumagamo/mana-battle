...
export default class CombatScene extends Phaser.Scene {
            ...
            squad.members
                ...
                .forEach(member => {
                    ...
                    const { x, y } = cartesianToIsometricBattle(
                        ...
                        isTopSquad
                            ? member.x
                            : Squad.invertBoardPosition(member.x),
                        isTopSquad
                            ? member.y
                            : Squad.invertBoardPosition(member.y)
                    )
    ...
    moveUnit(sourceId: string, targetId: string) {
        ...
        const { x, y } = cartesianToIsometricBattle(
            ...
            targetIsTop
                ? member.x + 1
                : Squad.invertBoardPosition(member.x) - 1,
        ...
        const timeEvents = {
            ...
            callback: () => {
                this.charas.sort(
                    (a, b) => a.container.depth - b.container.depth
                )
            },
            repeat: WALK_FRAMES,
        }
    ...
    async retreatUnits() {
        return this.charas.map(chara => {
            ...
            const getTargetPos = () => {
                ...
                if (charaIsTop) {
                    return {
                        x: chara.container.x - 200,
                        y: chara.container.y - 200,
                    }
                } else
                    return {
                        x: chara.container.x + 200,
    ...
    slash(
        ...
        return new Promise<void>(async resolve => {
            ...
            await delay(this, 1000 / GAME_SPEED)
            resolve()
        })
    }
