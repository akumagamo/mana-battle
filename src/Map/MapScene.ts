...
export class MapScene extends Phaser.Scene {
  ...
  update() {
    if (!this.isPaused) {
      let movedSquads = [] as string[];

      this.squadsInMovement.forEach((value, squadId) => {
        movedSquads.push(squadId);
        ...
        if (dist > 10) {
          ...
          if (remaining.length > 0) {
            console.log(
              `removing checkpoint, as there are checkpoins remaining`
            );
            this.squadsInMovement = this.squadsInMovement.set(squadId, {
              path: remaining,
              current,
      ...
      // check collision
      // TODO: each squad, store lists of enemies then compare
