...
export class ListSquadsScene extends Phaser.Scene {
    ...
    create({ squads, units, dispatched, onReturnClick }: CreateParams) {
        ...
        createSquadButtonClicked(this).on(
            this.handleCreateSquadClicked.bind(this)
        )
    ...
    renderSelectSquadInfo(squadId: string) {
        ...
        const baseX = 20
        const baseY = 610
        const panel_ = panel(20, baseY, SCREEN_WIDTH - 40, 80, this.uiContainer)
        ...
        button(baseX + 1000, baseY + 20, "Edit", this.uiContainer, () =>
        ...
        EditSquadModal({
            ...
            onClose: (sqd) => {
                ...
                sqd.members.forEach((m) => {
                    this.unitSquadIndex = this.unitSquadIndex.update(
                        m.id,
                        () => sqd.id
                    )
                })
